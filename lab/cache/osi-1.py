...
def cache_metrics():
    ...
    for i in range(1048576, 8388608, 1048576):
        sp = run_stress_ng(f'sudo perf stat -e cache-references,cache-misses stress-ng --cache 1 --prefetch-l3-size {i} --timeout 20')
        output = sp.communicate()[1]
        print(output)
        output = output.split('\n')
        refs = output[6]
    ...
    plot_graph([i for i in range(1048576, 8388608, 1048576)], cache_references_num, 'l3-refs', True)
    plot_graph([i for i in range(1048576, 8388608, 1048576)], cache_misses_percent, 'l3-misses', True)
